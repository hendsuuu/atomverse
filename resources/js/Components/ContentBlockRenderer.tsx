        ...
        case 'quote':
            return (
                <blockquote className="border-l-4 border-primary-400 pl-5 py-3 my-8 bg-primary-50/30 rounded-r-xl">
                    <p className="text-surface-700 italic text-lg leading-relaxed">"{block.content}"</p>
                    {block.author && (
                        <cite className="text-sm text-surface-500 mt-2 block not-italic">— {block.author}</cite>
                    )}
                </blockquote>
            );
...
function Callout({ variant, content }: { variant: string; content: string }) {
    const styles = {
        ...
        tip: { bg: 'bg-emerald-50', border: 'border-emerald-400', icon: '💡', text: 'text-emerald-800' },
        note: { bg: 'bg-purple-50', border: 'border-purple-400', icon: '📌', text: 'text-purple-800' },
    };
